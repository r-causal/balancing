...
Use `ipw()` when the method carries estimating equations, which is the estimating-equation family with a binary exposure and exact balance. It is faster and needs no tuning. Use the bootstrap for the quadratic-program methods, for an entropy fit at a positive tolerance, for an over-identified `bw_cbps()` fit, and for exposure types beyond binary. Covariate-adjusted outcome models need neither route in particular: `ipw()` stacks them directly. The bootstrap is more general but more expensive, and its precision improves with the number of resamples.
