...
The estimating-equation methods (`bw_entropy()`, `bw_ipt()`, and the just-identified `bw_cbps()`) fit weights that solve smooth estimating equations. That structure lets the uncertainty be propagated analytically: `propensity::ipw()` stacks the weight-parameter estimating equations, the outcome-model score equations, and the marginal-mean equations into one system and takes its sandwich variance. The result is a standard error that accounts for having estimated the weights.
...
propensity::ipw(sbw_fit, outcome_mod)
```
