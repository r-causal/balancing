...
A categorical exposure works the same way, with one marginal mean per level. The stacked system carries all of them, and the table reports each of those means and then the contrasts of each non-reference level against the reference level, which is the first of the exposure's own levels. The outcome model enters the exposure as a factor.
