...
A categorical exposure that `causalgenerics::joint_exposure()` declares as a crossing of two treatments is reported in those treatments instead of cell against cell: the cell means, each treatment's simple effects within the levels of the other, and their interaction. See `?ipw.balancing` for that surface. `ipw()` also takes a `.by` argument, which reports the effects again within the levels of a modifier and contrasts the subgroups, documented on the same page.
