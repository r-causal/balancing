...
#'
#' `sbw()` specifies stable balancing weights for [balance()]. Among all
...
#'
#' @return An `sbw` specification, a [balance_method].
#'
#' @references
...
method(method_label, sbw) <- function(method) {
  "Stable balancing weights"
}

method(supported_exposure_types, sbw) <- function(method) {
  c("binary", "categorical", "continuous")
}

method(supported_estimands, sbw) <- function(method, exposure_type) {
  switch(
    exposure_type,
...
method(supports_estimating_equations, sbw) <- function(
  method,
  ...,
  exposure_type = NULL,
  constraints = NULL
...
method(default_constraints, sbw) <- function(method) {
  ...
  if (!any(prepared$tolerances > 0)) {
    abort(
      c(
        ...
        i = "Set {.arg tolerance} in {.fn balance_terms} to a positive value, the central tuning parameter for {.fn sbw}."
      ),
  ...
  if (!identical(method@norm, "l2")) {
    abort(
      c(
        ...
        i = "Set {.code norm = \"l2\"} in {.fn sbw}; only the least-squares norm is solved in this version."
      ),
...
method(fit_method, sbw) <- function(method, prepared) {
