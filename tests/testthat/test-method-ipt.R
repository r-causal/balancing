# ipt() is the method spec; balance(..., method = ipt()) fits it. Inverse
...
test_that("ipt() carries its documented defaults", {
  spec <- ipt()
  expect_true(S7::S7_inherits(spec, ipt))
...
test_that("ipt() stores supplied tuning parameters", {
  spec <- ipt(
    link = "probit",
...
test_that("ipt() matches the link argument", {
  expect_identical(ipt(link = "cloglog")@link, "cloglog")
  expect_error(ipt(link = "identity"))
})

test_that("ipt() rejects unnamed extra arguments", {
  expect_true(S7::S7_inherits(ipt(), balance_method))
  expect_error(ipt(bogus = 1))
})

# ---- Validators -----------------------------------------------------------

test_that("ipt() rejects a non-positive convergence tolerance", {
  expect_identical(ipt()@convergence_tolerance, 1e-10)
  expect_error(ipt(convergence_tolerance = -1e-10))
})

test_that("ipt() rejects a negative iteration cap", {
  expect_null(ipt()@max_iterations)
  expect_error(ipt(max_iterations = -5L))
})
...
test_that("supported_exposure_types() excludes continuous", {
  expect_setequal(
    supported_exposure_types(ipt()),
    c("binary", "categorical")
  )
  expect_false("continuous" %in% supported_exposure_types(ipt()))
})

test_that("supported_estimands() depends on the exposure type", {
  binary <- supported_estimands(ipt(), "binary")
  expect_true(all(c("ate", "att") %in% binary))
  expect_true(any(c("atc", "atu") %in% binary))
  expect_false("ato" %in% binary)

  expect_setequal(
    supported_estimands(ipt(), "categorical"),
    c("ate", "att")
  )
})

test_that("supports_estimating_equations() is always TRUE for ipt", {
  expect_true(supports_estimating_equations(ipt()))
...
test_that("method_label() names the method", {
  expect_identical(method_label(ipt()), "Inverse probability tilting")
})

# ---- Statistical promises: binary -----------------------------------------

test_that("ipt balances a binary ate", {
  data <- sim_binary()
  fit <- balance(
    data,
    exposure,
...
test_that("ipt balances a binary att", {
  data <- sim_binary()
  fit <- balance(
    data,
    exposure,
...
test_that("ipt balances a binary atc", {
  data <- sim_binary()
  fit <- balance(
    data,
    exposure,
...
test_that("ipt balances a categorical ate", {
...
test_that("ipt balances a categorical att", {
  data <- sim_categorical()
  fit <- balance(
    data,
    exposure,
...
test_that("ipt balances a binary ate under sampling weights", {
  data <- sim_binary()
  withr::local_seed(21)
...
test_that("ipt balances a binary att under sampling weights", {
  data <- sim_binary()
  withr::local_seed(22)
  data$sw <- stats::runif(nrow(data), 0.3, 3)
  fit <- balance(
...
test_that("ipt balances a categorical att under sampling weights", {
  data <- sim_categorical()
  withr::local_seed(23)
  data$sw <- stats::runif(nrow(data), 0.3, 3)
  fit <- balance(
...
test_that("ipt att weights equal entropy balancing att weights (binary)", {
...
test_that("ipt weights match WeightIt for a binary ate", {
  skip_on_cran()
...
test_that("ipt weights match WeightIt for a binary att", {
  skip_on_cran()
  skip_if_not_installed("WeightIt")
  # WeightIt's ipt method solves its moment equations through rootSolve, a
  # transitive backend balancing does not depend on directly.
...
test_that("an ipt fit prints its summary block", {
